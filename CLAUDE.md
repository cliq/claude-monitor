...
Both providers report `PostToolUse`, which restores `working` after an answered question or a completed approved tool without replacing the user's prompt preview. Both hook scripts forward the optional `SessionStart.source`; `compact` preserves the current state, elapsed timer, and background task count (manual compaction of a waiting session must stay waiting). Tool payloads go to Python over stdin, never via the environment: large tool results exceed the OS environment-size limit.
...
Claude schema v4 and Codex schema v2 add `PostToolUse`. Older schemas missing this new event must inspect as `outdated` so `HookMaintenance` can migrate them; a missing event at the current version still means `modifiedExternally`. Bump `CURRENT_PROJECT_VERSION` when changing the bundled hooks so launch maintenance deploys the scripts and upgrades already-managed directories. Codex requires `/hooks` review after its hook definitions change; do not bypass its trust flow.
